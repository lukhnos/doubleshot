...
(function() {
  var Access, Arr, Assign, Asyncrun, Base, Block, Call, Class, Closure, Code, Comment, Existence, Extends, For, IDENTIFIER, IDENTIFIER_STR, IS_STRING, If, In, Index, LEVEL_ACCESS, LEVEL_COND, LEVEL_LIST, LEVEL_OP, LEVEL_PAREN, LEVEL_TOP, Literal, METHOD_DEF, NEGATE, NO, Obj, Op, Param, Parens, RESERVED, Range, Return, SIMPLENUM, STRICT_PROSCRIBED, Scope, Slice, Splat, Submodule, Switch, TAB, THIS, Throw, Try, UTILITIES, Value, While, YES, compact, del, ends, extend, flatten, last, merge, multident, nulllog, some, starts, unfoldSoak, utility, _ref, _ref1,
    __hasProp = {}.hasOwnProperty,
    __extends = function(child, parent) { for (var key in parent) { if (__hasProp.call(parent, key)) child[key] = parent[key]; } function ctor() { this.constructor = child; } ctor.prototype = parent.prototype; child.prototype = new ctor(); child.__super__ = parent.prototype; return child; },
    __indexOf = [].indexOf || function(item) { for (var i = 0, l = this.length; i < l; i++) { if (i in this && this[i] === item) return i; } return -1; };

  nulllog = function(str) {};
  ...
  exports.Block = Block = (function(_super) {
    ...
    Block.prototype.compileRoot = function(o) {
      var c, code, exp, i, mainPrelude, modulePrelude, n, prelude, preludeExps, rest, _ref2;
      nulllog("root options received: " + (JSON.stringify(o)));
      o.root = this;
      this.submoduleNames = {};
      this.submoduleCode = {};
      this.unnamedModuleCounter = 0;
      this.unnamedMoudleBasename = o.basename + "-worker-";
      modulePrelude = "";
      o.indent = o.bare ? '' : TAB;
      ...
      if (o.bare) {
        return "" + mainPrelude + "code";
      }
      return "" + mainPrelude + prelude + "(function() {\n" + code + "\n}).call(this);\n";
    };
    ...
    Block.prototype.declareModule = function(name) {
      nulllog("declared module: " + name);
      if (this.submoduleNames[name]) {
        return false;
      }
      this.submoduleNames[name] = true;
      return true;
    };

    Block.prototype.assignModuleName = function() {
      var name;
      this.unnamedModuleCounter++;
      name = this.unnamedMoudleBasename + this.unnamedModuleCounter + ".js";
      this.declareModule(name);
      nulllog("assigned module name: " + name);
      return name;
    };

    Block.prototype.storeModuleCode = function(name, code) {
      nulllog("storing module \"" + name + "\", length: " + code.length);
      return this.submoduleCode[name] = code;
    };
  ...
  exports.Submodule = Submodule = (function(_super) {

    __extends(Submodule, _super);

    function Submodule(name, code) {
      var capture, match;
      this.name = name;
      this.code = code;
      if (this.name) {
        capture = /^\"(.+)\"$/g;
        match = capture.exec(this.name);
        this.name = match[1];
        if (!/.+\.js$/.exec(this.name)) {
          this.name += ".js";
        }
      }
    }

    Submodule.prototype.isAssignable = YES;

    Submodule.prototype.makeReturn = function(res) {
      return this;
    };

    Submodule.prototype.compileNode = function(o) {
      var actualCode, check, prelude;
      if (!o.worker && !o.nodeprocess) {
        throw new Error('program that uses submodule must be compiled with -W or -N');
      }
      if (this.name) {
        check = o.root.declareModule(this.name);
        if (!check) {
          throw new Error("duplicate submodule name: " + this.name);
        }
      } else {
        this.name = o.root.assignModuleName();
      }
      o.scope = new Scope(null, this.code, null);
      o.level = LEVEL_TOP;
      o.indent = '';
      prelude = "";
      if (o.worker) {
        prelude = "// DoubleShot HTML5 worker setup code\nvar moduleSelf = { name: \"" + this.name + "\", loadPath: \"\" };\nmoduleSelf.receive = function(m) {};\nmoduleSelf.reply = function(m) { postMessage(m); };\nmoduleSelf.close = function() { close(); };\nthis.onmessage = function(e) {\n  if (typeof e.data === \"object\" && typeof e.data._submoduleLoadPath == \"string\") {\n    moduleSelf.loadPath = e.data._submoduleLoadPath;\n  }\n\n  this.onmessage = function(e) {\n    moduleSelf.receive(e.data);\n  };\n};\n";
      } else if (o.nodeprocess) {
        prelude = "var moduleSelf = { name: \"" + this.name + "\" };\nmoduleSelf.receive = function(m) {};\nmoduleSelf.reply = function(m) { process.send(m); };\nmoduleSelf.close = function() { process.exit(0); };\nprocess.on('message', function(m) { moduleSelf.receive(m); });\n";
      }
      actualCode = prelude + (this.code.compileWithDeclarations(o));
      o.root.storeModuleCode(this.name, actualCode);
      if (o.worker) {
