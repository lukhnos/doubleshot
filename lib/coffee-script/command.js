...
(function() {
  ...
  SWITCHES = [['-b', '--bare', 'compile without a top-level function wrapper'], ['-c', '--compile', 'compile to JavaScript and save as .js files'], ['-e', '--eval', 'pass a string from the command line as input'], ['-h', '--help', 'display this help message'], ['-i', '--interactive', 'run an interactive CoffeeScript REPL'], ['-j', '--join [FILE]', 'concatenate the source CoffeeScript before compiling'], ['-l', '--lint', 'pipe the compiled JavaScript through JavaScript Lint'], ['-n', '--nodes', 'print out the parse tree that the parser produces'], ['--nodejs [ARGS]', 'pass options directly to the "node" binary'], ['-o', '--output [DIR]', 'set the output directory for compiled JavaScript'], ['-p', '--print', 'print out the compiled JavaScript'], ['-r', '--require [FILE*]', 'require a library before executing your script'], ['-s', '--stdio', 'listen for and compile scripts over stdio'], ['-t', '--tokens', 'print out the tokens that the lexer/rewriter produce'], ['-v', '--version', 'display the version number'], ['-w', '--watch', 'watch scripts for changes and rerun commands'], ['-N', '--nodeprocess', 'emit concurrency code using node.js process'], ['-W', '--worker', 'emit concurrency code using HTML5 worker (and produce worker files)']];
    ...
    try {
      ...
      if (o.tokens) {
        ...
        if (o.print) {
          ...
          return writeJsWithModules(t.file, t.output, output.modules, base);
        } else if (o.lint) {
  ...
  writeJsWithModules = function(source, js, modules, base) {
    var compile, jsDir, jsPath;
    jsPath = outputPath(source, base);
    jsDir = path.dirname(jsPath);
    compile = function() {
      var moduleJS, modulePath, name;
      for (name in modules) {
        moduleJS = modules[name];
        modulePath = path.join(jsDir, name);
        console.log("write HTML5 submodule: " + modulePath);
        fs.writeFile(modulePath, moduleJS, function(err) {
          if (err) {
            return printLine(err.message);
          } else if (opts.compile && opts.watch) {
            return timeLog("compiled " + source);
